...
cv_result = cross_val_score(model, new_train,df.y, cv=5, scoring='neg_log_loss')
...
# Single model
X_train, X_test, y_train, y_test = train_test_split(new_train,df.y.values,test_size=0.2,random_state=2048)
...
for train_idx, valid_idx in kf.split(X_test,y_test):
    ...
    model1.fit(train_data[0],train_data[1], eval_set=[(valid_data)],verbose=0,early_stopping_rounds=200)
    ...
    logging.info(f'semi-{count} mlogloss: {qq}, single mlogloss: {round(orig_score,5)}')
    gc.collect()
