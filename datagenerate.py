...
size = submit.shape[0]
with mp.Pool(core) as p:
    test_size = list(tqdm(p.imap(getDataSize_md5,submit.md5[:size]), total=size))
...
for max_seq in [x*10000 for x in range(6,50,5) ]:
    ...
    control_limit_md5=partial(control_limit,max_seq=max_seq,mode='train',thresholds=thresholds)
    size = df.shape[0]
    with mp.Pool(core) as p:
        train_control = list(tqdm(p.imap(control_limit_md5,df.md5[:size]), total=size))
    train_control = sparse.csr_matrix(train_control)
    logging.info(f'Generate test data {max_seq}')
    control_limit_md5=partial(control_limit,max_seq=max_seq,mode='test',thresholds=thresholds)
    size = submit.shape[0]
    with mp.Pool(core) as p:
        test_control = list(tqdm(p.imap(control_limit_md5,submit.md5[:size]), total=size))
